...
Live findings stream to stderr above a progress line like `[N/total] X% | rps | eta | url=https://example.com/admin | active=150 hosts=2`. Fuzzing is concurrent, so `url` identifies the newest request still in flight and shows exactly how the current dictionary path was appended to its target; `active` and `hosts` show the current number of requests and target hosts in flight. Long URLs retain both their beginning and path tail when shortened, and terminal control characters are neutralized. `--no-live` hides findings only; `-q` hides findings and progress while retaining phase summaries.
